...
// Requête de sélection MySQL pour le titre de la page
$query = $db->prepare(
  "SELECT *
    FROM genre
    WHERE genre_id LIKE '$id'"
);
$query->execute();
...
$query = $db->prepare(
  "SELECT *
    ...
    INNER JOIN genre_link_tvshow ON art.media_id = genre_link_tvshow.media_id
    WHERE genre_id LIKE '$id'
...
<head>
  <title><?php foreach ($title as $row) {
            echo 'Séries genre ' . mb_strtolower($row['name'], 'UTF-8') . '';
          } ?></title>
  <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="Description" content="Séries par genre">
